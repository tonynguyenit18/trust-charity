...
class AdminCheckPostModal extends React.Component {
  ...
  componentDidMount() {
    const getSalerUrl = `http${process.env.REACT_APP_APP_DOMAIN ? 's' : ""}://${process.env.REACT_APP_APP_DOMAIN || 'localhost:8000'}/user?role=wholesaler`
  ...
  rejectHandle = () => {
    const postUpdateUrl = `http${process.env.REACT_APP_APP_DOMAIN ? 's' : ''}://${process.env.REACT_APP_APP_DOMAIN || 'localhost:8000'}/posts/${this.props.postObj._id}`
